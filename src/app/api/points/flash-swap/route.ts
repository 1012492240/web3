...
import { flashSwap, innerTransfer } from '@/lib/balance';
...
export async function POST(req: NextRequest) {
    ...
    const { amount, walletAddress, timestamp, signature, operationType } = await req.json();
    if (!amount || amount <= 0) {
    ...
    try {
      ...
      return NextResponse.json({
        message: 'Flash swap successfully'
      });
    } catch (error) {
      console.error('Error flash swap:', error);
      return NextResponse.json(
